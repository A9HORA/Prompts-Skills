...
> **Version:** 3.0
> **Categories:** 70 (37 base + 33 platform-specific)
> **Scope:** Backend, frontend, mobile, AI/LLM, scripts, CI/CD, infrastructure
...
Before scanning, detect the project type from manifest files (`package.json`, `build.gradle`, `Podfile`, `Cargo.toml`, `go.mod`, `requirements.txt`, `pom.xml`, `pubspec.yaml`) and directory structure. Apply **Base categories (1–37)** to every project. Then apply the relevant **platform module(s)**:

- **AI/LLM integration detected** (openai, anthropic, langchain, llama-index, transformers, autogen, crewai, semantic-kernel, google-generativeai, cohere, replicate in deps or imports): also apply **categories 38–47**
- **Mobile** (React Native, Flutter, Android/Kotlin/Java, iOS/Swift/ObjC): also apply **categories 48–55**
- **Web frontend with SSR** (Next.js, Nuxt, Remix, SvelteKit, Astro, or any server-rendered HTML templates): also apply **categories 56–62**
- **Web frontend SPA-only** (React, Vue, Angular, Svelte without SSR): also apply **categories 59–62**
- **Scripts / CLI / infrastructure** (Dockerfile, docker-compose, Terraform/Pulumi, Helm/K8s manifests, CI configs, shell/python/ruby scripts): also apply **categories 63–70**
...
SQL queries built with string concatenation or template literals. `child_process.exec` / `subprocess.run(shell=True)` / `os.system()` / backtick execution with interpolated user input. `eval()`, `new Function()`, `vm.runInContext()` with tainted strings. NoSQL operator injection (`$gt`, `$ne` in MongoDB queries from user input). Template injection in Jinja2, Pug, EJS, Handlebars, Twig when user input reaches template compilation (not just variable rendering).
...
API keys, passwords, tokens, signing secrets, database connection strings in source files, config files (`pcln.json`, `appsettings.json`, `.env` committed to repo), or CI/CD configs. Include: base64-encoded credentials in headers, default passwords that are never rotated, secrets in Dockerfiles or docker-compose.yml. Check `pcln.json`, `config/`, `.github/workflows/`, `Jenkinsfile`, `Dockerfile`, `docker-compose*.yml`, `terraform.tfvars`, `.env*`.
...
`Object.assign(model, req.body)`, spread operator `{...req.body}` into database models, Mongoose `new Model(req.body)`, Sequelize `Model.create(req.body)`, Rails `params.permit` missing or overly broad. Allows setting `isAdmin`, `role`, `price`, `balance` from request body.
...
Weak hashes for passwords (MD5, SHA1, unsalted). Insecure modes (ECB). Hardcoded IVs/nonces. Nonce reuse in AES-GCM/ChaCha20. `Math.random()` or non-CSPRNG sources for tokens, session IDs, or keys. Missing constant-time comparison for secret/token verification (`===` instead of `crypto.timingSafeEqual`).
...
String interpolation into HTML attributes (`href`, `src`, `style`, `on*` handlers, `data-*`) without context-aware escaping. Check: template literals in `.ts`/`.js` files building HTML strings, EJS/Pug/Handlebars/Jinja2 with unescaped output (`<%- %>`, `!{}`, `{{{ }}}`, `| safe`). Single-quote or double-quote breakout in attribute values. `javascript:` scheme in `href`/`src` attributes rendered from user input.
...
When two findings combine to form a more dangerous exploit chain, escalate severity. Apply every matching rule — multiple can fire on the same finding.

| Rule | Trigger combination | Escalation | Rationale |
|------|---------------------|------------|-----------|
| **CSP × Injection** | CSP ineffective (cat 32) + any injection (cats 1, 34, 35, 36, 56, 57, 58) | Injection finding escalates one level (Medium→High, High→Critical) | Weak CSP removes the last defense; injection becomes guaranteed script execution. |
| **PCI × Auth bypass** | PAN/cardholder data handling (cat 16) + missing auth or auth bypass (cats 7, 8, 10, 11) | Escalate to Critical | Unauthenticated access to PAN triggers mandatory breach notification and PCI scope expansion. |
| **Injection × Cookie flags** | Any injection finding (cats 1, 34, 35, 56, 57, 58) + auth cookies missing `httpOnly` (cat 14) | Injection finding escalates one level | Injection + readable auth cookies = full session hijack / account takeover chain. |
| **IDOR × Sensitive data** | IDOR (cat 9) on endpoint returning/modifying payment methods, PAN, SSN, medical records, or auth credentials | Escalate to Critical | Data sensitivity multiplies impact beyond generic IDOR. Note the data type in the finding. |
| **Open redirect × OAuth** | Open redirect (cat 4) on an OAuth `redirect_uri` callback endpoint (cat 8) | Escalate to Critical | Attacker steals authorization codes via redirect, yielding victim's access token. |
| **Prompt injection × Destructive tools** | Prompt injection (cat 38) + uncontrolled tool calling (cat 39) where tools include destructive actions (delete, send, pay, modify, write) | Escalate to Critical | Prompt injection with destructive tool access is RCE-equivalent — attacker executes arbitrary actions under the user's identity. |
| **Proxy trust × Host-header sink** | Proxy misconfiguration (cat 20) + any finding using `req.hostname`/`req.ip`/`req.protocol` for redirect (cat 4), URL construction (cat 6), or security decisions (cat 11) | Co-occurring finding escalates one level | Proxy trust is the enabler; without it the downstream bug may be unexploitable. |
| **Race condition × Financial op** | TOCTOU (cat 23) on payment, balance, coupon, credit, quota, or inventory operation | Escalate to Critical | Double-spend, coupon replay, or balance manipulation has direct financial impact. |
| **Secrets exposure × Public reach** | Hardcoded secret (cat 12) used by an unauthenticated endpoint (cat 7), exposed in client bundle (cat 13), or in a public repository | Escalate to Critical | Secret is actively exploitable without any precondition — rotate immediately. |
| **Multi-agent injection × Destructive agent** | Compromised agent output (cat 44) reaching an agent with destructive tools (cat 39) | Escalate to Critical | One poisoned agent cascades to destructive actions in another; blast radius multiplied by agent count. |
| **Supply chain × CI/CD** | Malicious install script or unpinned action (cat 28) in a CI/CD pipeline (cat 63) that has access to production secrets or deployment credentials | Escalate to Critical | Supply chain compromise in CI/CD = arbitrary code execution with production access. |
...
| Review type | Static source and configuration review (70-category scope) |
...
> - **Execution flow**: Numbered path from public request entry to sink (max 8 steps), each with `file:line`. Does not repeat the full Scope file list.
>
...
**CVSS 3.1 Base** — If confidence is HIGH, produce a full vector string. Show reasoning for each metric (AV, AC, PR, UI, S, C, I, A) in one line each. Mark uncertain metrics.

**Scope** — Three-tier table:
...
**Execution flow** — Numbered steps (max 8), each citing `file:line`.

**Issue details** — Mechanism (2–3 sentences). Preconditions (what attacker needs).

**Implicit assumption violated** — One sentence stating what the code assumes about its inputs or environment that an attacker can break. **Required** for findings in categories 7, 8, 9, 10, 11, 16, 23, 37, 42, 44 and their platform-module aliases.
